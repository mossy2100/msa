...
//~ {
	...
	//~ // envoie du mail
	//~ $res = @mail( $this->strTo, $this->xheaders['Subject'], $this->fullBody, $this->headers, "-f ".$this->xheaders["From"] );
	//~ // FIXME addition of "-f" parameter by Sebastian Held <sebastian.held@uni-due.de>
	//~ // FIXME NET YET APPROVED by Prof. Lovell; don't know of side effects
	//~ // this sets the envelope sender to the "From"-Address; required by our data processing center
	//~ // it also allows us to receive bounces from illegal mail addresses, ...
...
//~ {
	//~ if( ereg( ".*<(.+)>", $address, $regs ) ) {
		//~ $address = $regs[1];
	//~ }
 	//~ if(eregi( "^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,6})$",$address) ) 
 		//~ return true;
 	//~ else
 		//~ return false;
//~ }
