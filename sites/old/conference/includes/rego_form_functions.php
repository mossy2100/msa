...
function vfValidate($valFunction, &$fieldValue, &$message, $db = NULL, $acceptedCards = "") {
	switch ($valFunction) {
		...
		case 21: // registered username
			...
			return $valid;
			break;
		case 22: // registered author
			$rsVal = $db->Execute("SELECT CONCAT_WS(' ', FirstName, MiddleName, LastName) AS Author" .
			                      "FROM " . $GLOBALS['DB_PREFIX'] . "Member " .
			                      "WHERE CONCAT_WS(' ', FirstName, MiddleName, LastName) = '" . $fieldValue . "'");
			if ($valid = ($val = $rsVal->FetchRow())) {
				// correct case of field value by retrieving it from database
				$fieldVal = $val['Author'];
			} else
			...
			return $valid;
			break;
		case 23: // registered paper name
			$rsVal = $db->Execute("SELECT Title " .
			                      "FROM " . $GLOBALS['DB_PREFIX'] . "Paper " .
			                      "WHERE Title = '" . $fieldValue . "'");
			if ($valid = ($val = $rsVal->FetchRow())) {
				// correct case of field value by retrieving it from database
				$fieldVal = $val['Title'];
			} else
		...
		case 31: // credit card number
			...
			break;
		case 32: // credit card expiry date
			$valid = ccCheckExpiry($fieldValue, $message);
			if (!$valid)
				$message = ($message > "" ? $message : "Invalid expiry date.");
			return $valid;
			break;
		default: // invalid validation function, assume correct
			return true;
			break;
	}
}
