...
class Blogger_Import {
	...
	function greet() {
		...
		if ( function_exists('curl_init') )
			echo "<iframe src='/blog/wp-admin/admin.php?import=blogger&amp;noheader=true' height='350px' width = '99%'>$noiframes</iframe><p><a href='/blog/wp-admin/admin.php?import=blogger&amp;restart=true&amp;noheader=true' onclick='return confirm(\"$warning\")'>$reset</a></p>";
		else
			echo "<p>$incompat</p>";
		echo "</div>\n";
	}
	...
	function show_blogs() {
		...
		foreach ( $this->import['blogs'] as $blog ) {
			...
			echo "\t<li><a href='/blog/wp-admin/admin.php?import=blogger&amp;noheader=true&amp;blog={$blog['id']}'>{$blog['title']}</a> $status</li>\n";
		}
		die("</ul>\n");
	}
